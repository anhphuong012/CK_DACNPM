...
//    }
	...
	@PostMapping("")
	ResponseEntity<ReposeOject> addShedule(@RequestBody SheduleDTO shedule) {

		SheduleDTO data = iSheduleService.addShedule(shedule);

		if (data != null) {
			return ResponseEntity.status(HttpStatus.OK).body(new ReposeOject("OK", "Query Shedule successfully", data));
		} else {
			return ResponseEntity.status(HttpStatus.OK).body(new ReposeOject("failed", "Error", data));
		}

	}

	@GetMapping("/{id}")
	ResponseEntity<ReposeOject> getSheduleByDoctor(@PathVariable Long id,
			@RequestParam(value = "date", required = false) List<String> date) {

		List<SheduleDTO> data = iSheduleService.findByDateAndDoctor(id, date);
	...
	@DeleteMapping("/{id}")
	ResponseEntity<ReposeOject> deleteById(@PathVariable Long id) {
		boolean check = iSheduleService.deleteShedule(id);
		if (check) {
			return ResponseEntity.status(HttpStatus.OK)
					.body(new ReposeOject("OK", "Query Shedule successfully", check));
		} else {
			return ResponseEntity.status(HttpStatus.OK).body(new ReposeOject("failed", "Error", check));
		}
	}
